...
function load_wp_posts() {
    jQuery.get(ajaxurl + "/wp-json/pricode/v1/movies/", function (response) {
        var posts = response.data;
        console.log(posts);
        var html = "";
        jQuery.each(posts, function (index, post) {
            html += '<tr><td>' + (index + 1) + '</td><td>' + post.post_title + '</td><td>' + post.post_content + '</td><td>publish</td><td><a class="btn btn-info post-edit" href="javascript:void(0)" data-toggle="modal" data-target="#postEdit" data-id="' + post.ID + '">Edit</a><a class="btn btn-danger post-delete" href="javascript:void(0)" data-id="' + post.ID + '">Delete</a></td></tr>';
        });
